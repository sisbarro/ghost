...
def _render_pdf_reportlab(html_template: str, context: dict, filename: str) -> bytes:
    """Legacy renderer used when xhtml2pdf is unavailable."""
...
# Wraps user content in browser-print styling so PDFs render like Save as PDF from a browser.
_PDF_DOCUMENT_TEMPLATE = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>{title}</title><style>
@page {{ size: letter; margin: 0.85in 0.9in 1.1in 0.9in;
    @frame footer {{ -pdf-frame-content: gm-footer; left: 0.9in; right: 0.9in; bottom: 0.45in; height: 0.4in; }} }}
body {{ font-family: Helvetica, Arial, sans-serif; color: #1f2937; font-size: 11pt; line-height: 1.55; }}
h1, h2, h3, h4 {{ color: #111827; font-weight: 700; margin: 0 0 8pt; }}
h1 {{ font-size: 22pt; padding-bottom: 6pt; border-bottom: 1.5pt solid #4f4ee6; margin-bottom: 14pt; }}
h2 {{ font-size: 16pt; margin-top: 16pt; }}
h3 {{ font-size: 13pt; margin-top: 12pt; color: #374151; }}
h4 {{ font-size: 11.5pt; margin-top: 10pt; color: #4b5563; }}
p {{ margin: 0 0 9pt; }}
strong, b {{ color: #111827; }}
em, i {{ color: #374151; }}
a {{ color: #4f4ee6; text-decoration: none; }}
ul, ol {{ margin: 0 0 10pt 18pt; padding: 0; }}
li {{ margin-bottom: 3pt; }}
blockquote {{ margin: 12pt 0; padding: 6pt 12pt; border-left: 3pt solid #6370f2;
    background: #f8f9ff; color: #4b5563; font-style: italic; }}
hr {{ border: none; border-top: 0.5pt solid #d1d5db; margin: 14pt 0; }}
code {{ font-family: Courier, monospace; background: #f3f4f6; padding: 1pt 4pt;
    border-radius: 2pt; font-size: 9.5pt; color: #4f4ee6; }}
pre {{ font-family: Courier, monospace; background: #f9fafb; border: 0.5pt solid #e5e7eb;
    padding: 8pt 10pt; border-radius: 3pt; font-size: 9.5pt; line-height: 1.4; }}
table {{ width: 100%; border-collapse: collapse; margin: 10pt 0 12pt; }}
th {{ background: #f3f4f6; color: #111827; text-align: left; padding: 7pt 10pt;
    border-bottom: 1pt solid #d1d5db; font-weight: 700; font-size: 10pt; }}
td {{ padding: 6pt 10pt; border-bottom: 0.5pt solid #e5e7eb; vertical-align: top; }}
tr:nth-child(even) td {{ background: #fbfbfc; }}
img {{ max-width: 100%; }}
.gm-footer {{ font-size: 8pt; color: #9ca3af; text-align: center;
    border-top: 0.3pt solid #e5e7eb; padding-top: 4pt; }}
</style></head><body>
{body}
<div id="gm-footer" class="gm-footer">Generated by GhostMail &middot; <pdf:pagenumber> of <pdf:pagecount></div>
</body></html>"""


def _render_pdf_xhtml2pdf(html_template: str, context: dict, filename: str) -> bytes | None:
    """Render user HTML wrapped in a print-quality stylesheet; None on failure."""
    try:
        from xhtml2pdf import pisa
    except ImportError:
        return None
    rendered_body = process_template(html_template, context)
    html = _PDF_DOCUMENT_TEMPLATE.format(title=escape(filename), body=rendered_body)
    output = BytesIO()
    try:
        result = pisa.CreatePDF(html, dest=output, encoding="utf-8")
    except Exception as exc:
        logger.warning(f"xhtml2pdf render error: {exc}")
        return None
    if getattr(result, "err", 0):
        logger.warning(f"xhtml2pdf reported {result.err} error(s); falling back to ReportLab.")
        return None
    return output.getvalue()
...
def render_pdf_attachment(config: dict | None, context: dict) -> dict | None:
    ...
    pdf_bytes = _render_pdf_xhtml2pdf(html_template, context, filename)
    if pdf_bytes is None:
        pdf_bytes = _render_pdf_reportlab(html_template, context, filename)
